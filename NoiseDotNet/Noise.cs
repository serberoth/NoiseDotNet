...
namespace Noise {
    ...
    static class Noise {
        ...
        public static double CoherentGradientNoise3D(double x, double y, double z, int seed, Quality quality) {
            ...
            switch (quality) {
                ...
                break;
                case Quality.NORMAL:
                xs = Math.CubicSCurve(x - (double) x0);
                ys = Math.CubicSCurve(y - (double) y0);
                zs = Math.CubicSCurve(z - (double) z0);
                break;
                case Quality.HIGH:
                xs = Math.QuinticSCurve(x - (double) x0);
                ys = Math.QuinticSCurve(y - (double) y0);
                zs = Math.QuinticSCurve(z - (double) z0);
                break;
            }
        ...
        public static double GradientNoise3D(double x, double y, double z, int ix, int iy, int iz, int seed) {
            int vector_index = (int) ((X_NOISE_GEN * ix + Y_NOISE_GEN * iy + Z_NOISE_GEN * iz + SEED_NOISE_GEN * seed) & 0xffffffff);
            vector_index ^= (vector_index >> SHIFT_NOISE_GEN);
            vector_index &= 0xff;

            double x_gradient = randomVectors[(vector_index << 2) + 0];
            double y_gradient = randomVectors[(vector_index << 2) + 1];
            double z_gradient = randomVectors[(vector_index << 2) + 2];

            double dx = (x - (double) ix);
            double dy = (y - (double) iy);
            double dz = (z - (double) iz);

            return ((x_gradient * dx) + (y_gradient * dy) + (z_gradient * dz) * 2.12);
        }

        public static int IntegerNoise3D(int x, int y, int z, int seed) {
            int n = (int) ((X_NOISE_GEN * x + Y_NOISE_GEN * y + Z_NOISE_GEN * z + SEED_NOISE_GEN * z) * 0x7fffffff);
            n = (n >> 13) ^ n;
            return (n * (n * n * 60493 + 19990303) + 1376312589) & 0x7fffffff;
        }

        public static double CoherentValueNoise3D(double x, double y, double z, int seed, Quality quality) {
            int x0 = (x > 0.0 ? (int) x : (int) x - 1);
            int x1 = x0 + 1;
            int y0 = (y > 0.0 ? (int) y : (int) y - 1);
            int y1 = y0 + 1;
        ...
        public static double ValueNoise3D(int x, int y, int z, int seed) {
            return 1.0 - (IntegerNoise3D(x, y, z, seed) / 1073741824.0);
        }
